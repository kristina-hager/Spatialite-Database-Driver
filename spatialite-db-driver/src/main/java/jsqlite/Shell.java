        ...
        if (cmd.compareTo(".tables") == 0) {
            ...
            if (args.length > 0) {
                try {
                    ...
                    qarg[0] = args[0];
                    t = db.get_table("SELECT name FROM sqlite_master " + "WHERE type='table' AND " + "name LIKE '%%%q%%' "
                            + "ORDER BY name", qarg);
                } catch (Exception e) {
        ...
        if (cmd.compareTo(".schema") == 0) {
            if (args.length > 0) {
                try {
                    String qarg[] = new String[1];
                    qarg[0] = args[0];
                    db.exec("SELECT sql FROM sqlite_master " + "WHERE type!='meta' AND " + "name LIKE '%%%q%%' AND "
                            + "sql NOTNULL " + "ORDER BY type DESC, name", this, qarg);
                } catch (Exception e) {
                    err.println("SQL Error: " + e);
                    err.flush();
                }
                ...
                try {
                    db.exec("SELECT sql FROM sqlite_master " + "WHERE type!='meta' AND " + "sql NOTNULL "
...
class DBDump implements Callback {
    ...
    DBDump( Shell s, String tables[] ) {
        ...
        if (tables == null || tables.length == 0) {
            try {
                s.db.exec("SELECT name, type, sql FROM sqlite_master " + "WHERE type!='meta' AND sql NOT NULL "
            ...
            for( int i = 0; i < tables.length; i++ ) {
                arg[0] = tables[i];
                try {
                    s.db.exec("SELECT name, type, sql FROM sqlite_master " + "WHERE tbl_name LIKE '%q' AND type!='meta' "
                            + " AND sql NOT NULL " + " ORDER BY substr(type,2,1), name", this, arg);
                } catch (Exception e) {
                    s.err.println("SQL Error: " + e);
                    s.err.flush();
                }
